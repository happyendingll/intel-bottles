...
# targets.txt is a generated refresh queue, not the full maintained set. Every successful
# bottle is represented by an active manifest; when one becomes stale, apply_manifest emits
# HELD and the following build-bottles run refreshes it. New prewarm successes therefore join
# this maintenance loop automatically.
TARGETS_TMP="$(mktemp)"
trap 'rm -f "$TARGETS_TMP"' EXIT
{
  echo "# Generated by sync-fork from stale active manifests."
  echo "# The maintained set is manifest/*.bottle.json; do not edit this queue manually."
  echo
  printf '%s\n' "$PLAN" | awk -F '\t' '$1 == "HELD" { print $2 }' | LC_ALL=C sort -u
} > "$TARGETS_TMP"
mv "$TARGETS_TMP" "$REPO_DIR/targets.txt"
trap - EXIT
HELD_COUNT="$(awk 'NF && $1 !~ /^#/ { count++ } END { print count + 0 }' "$REPO_DIR/targets.txt")"
echo "==> wrote $HELD_COUNT held formulae to targets.txt"
